    ...
    return (
      <div className="container mx-auto px-4 py-20">
        <div className="text-center py-20">
          <h1 className="text-3xl font-bold text-white mb-4">Search LianoFlix</h1>
          <p className="text-neutral-400">Enter a search term to find movies and actors.</p>
        </div>
      </div>
    );
  }
        ...
        <div>
          {isLoadingMovies ? (
            ...
            <div className="text-center py-12">
              <p className="text-neutral-400">An error occurred while searching for movies.</p>
            </div>
          ) : movies.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-neutral-400">No movies found matching &quot;{query}&quot;.</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
              {movies.map((movie) => (
      ...
      {activeTab === 'people' && (
        <div>
          {isLoadingPeople ? (
            ...
            <div className="text-center py-12">
              <p className="text-neutral-400">An error occurred while searching for people.</p>
            </div>
          ) : people.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-neutral-400">No people found matching &quot;{query}&quot;.</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
              {people.map((person) => (

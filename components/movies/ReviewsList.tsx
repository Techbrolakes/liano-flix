...
export function ReviewsList({ movieId }: ReviewsListProps) {
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        {[1, 2, 3].map((i) => (
          <div key={i} className="bg-neutral-900/60 p-6 rounded-lg animate-pulse border border-neutral-800">
            ...
            <div className="mb-4">
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <div key={star} className="w-6 h-6 rounded-full bg-neutral-800" />
                ))}
    ...
    return (
      <div className="bg-red-900/20 border border-red-800 p-5 rounded-lg">
        <div className="flex items-center gap-3 text-red-500">
          ...
          <p className="font-medium">Error loading reviews. Please try again later.</p>
        </div>
      </div>
    );
  }
  ...
  if (!reviews || reviews.length === 0) {
    return (
      <div className="bg-neutral-900/60 p-6 rounded-lg border border-neutral-800 text-center">
        ...
        <p className="text-neutral-400 max-w-md mx-auto">Be the first to share your thoughts about this movie!</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-xl font-medium text-white mb-2">{reviews.length} {reviews.length === 1 ? 'Review' : 'Reviews'}</h3>
      {reviews.map((review) => (
        <div key={review.id} className="bg-gradient-to-b from-neutral-800/60 to-neutral-900/60 p-6 rounded-lg border border-neutral-700 backdrop-blur-sm shadow-md">
          ...
          <div className="mb-4">
            <div className="flex items-center gap-2 mb-1">
              <StarRating rating={review.rating} onChange={() => {}} readOnly size="md" />
              <span className="text-lg font-bold text-yellow-500 ml-1">{review.rating}/5</span>
            </div>
          ...
          <div className="bg-neutral-800/40 p-4 rounded-lg border border-neutral-700">
            <p className="text-neutral-200 whitespace-pre-wrap leading-relaxed">{review.comment}</p>
          </div>
        </div>
      ))}
    </div>
